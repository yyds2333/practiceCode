...
$(function(){
	...
	$.ajax({
		...
		error:function(data){//当访问时候，404，500 等非200的错误状态码
			validateTip(userRole.next(),{"color":"red"},imgNo+" Get userlist error",false);
		}
	});
	
	
	userName.on("focus",function(){
		validateTip(userName.next(),{"color":"#666666"},"* User name length must be >1 and <10",false);
	}).on("blur",function(){
		if(userName.val() != null && userName.val().length > 1 
				&& userName.val().length < 10){
			validateTip(userName.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(userName.next(),{"color":"red"},imgNo+" Please input again",false);
		}
	...
	birthday.on("focus",function(){
		...
		if(birthday.val() != null && birthday.val() != ""){
			...
			validateTip(birthday.next(),{"color":"red"},imgNo + " Please input again",false);
		}
	});
	...
	phone.on("focus",function(){
		...
		if(phone.val().match(patrn)){
			...
			validateTip(phone.next(),{"color":"red"},imgNo + " Format wrong!",false);
		}
	});
	
	userRole.on("focus",function(){
		validateTip(userRole.next(),{"color":"#666666"},"Select your role!",false);
	}).on("blur",function(){
		if(userRole.val() != null && userRole.val() > 0){
			validateTip(userRole.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(userRole.next(),{"color":"red"},imgNo+" Select your role again!",false);
		}
